...
class Gamemanager():
    ...
    def event_queue(self, player:Player.Player):
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                #Level-1的操作
                if event.key == pygame.K_ESCAPE and self.state == GameState.GAME_PLAY_LEVEL_1:
                    self.flush_scene(GameState.GAME_PLAY_PAUSE, player, 0, 0)
                if event.key == pygame.K_1 and self.state == GameState.GAME_PLAY_LEVEL_1:
                    self.flush_scene(GameState.GAME_PLAY_LEVEL_2, player, PosSettings.level_2_X, PosSettings.level_2_Y)

                #Level-2的操作
                if event.key == pygame.K_ESCAPE and self.state == GameState.GAME_PLAY_LEVEL_2:
                    self.flush_scene(GameState.GAME_PLAY_PAUSE, player, 0, 0)
    ...
    def update_enemy(self, player):
        for enemy in self.scene.enemies: enemy.update(player)
    ...
    def update(self, player:Player.Player):
        if self.state != GameState.GAME_PLAY_PAUSE:
            ...
            self.update_coin()
            self.update_enemy(player)

    def render(self):
        if self.state == GameState.GAME_PLAY_LEVEL_1 or self.state == GameState.GAME_PLAY_LEVEL_2 or self.state == GameState.GAME_PLAY_ORIGIN:
